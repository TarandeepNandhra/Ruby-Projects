...
# poluate groups with group_number amount of arrays
for _ in 1..group_number do
  groups.push([])
end

# assign each element of names to a group using modulo 
for i in 0...names.length do
  groups[i % group_number].push(names[i])
end
...
while true do
  ...
  if number.to_i <= group_number
    puts groups[number.to_i - 1].join(", ")
  end
